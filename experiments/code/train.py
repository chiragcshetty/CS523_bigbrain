...
if __name__ == "__main__":
    ...
    model = dm.parallelModelThreeLayerSplit(factor, split, 0)
    ...
    last_gpu = (
        output.get_device()
    )  ## GPU on which labbels should be placed for training
    ...
    #y_onehot = (torch.nn.functional.one_hot(y, num_classes=opt_size).type(torch.FloatTensor)).to(last_gpu)
    start = time.time()
    # with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA]) as prof:

    for (x, y) in data_loader:
        #labels = torch.randn((batch_size, opt_size)).to(last_gpu)
        y_pred = model(x); y_onehot = (torch.nn.functional.one_hot(y, num_classes=opt_size).type(torch.FloatTensor)).to(last_gpu)
