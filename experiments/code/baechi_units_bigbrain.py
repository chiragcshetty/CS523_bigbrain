...
class Args:
     def __init__(self,itype, prof_rounds, prof_gpu_id, gpu_num, sch): ## CHANGE: batch_size removed
         self.type = itype
...
args = Args(itype, prof_rounds, prof_gpu_id, gpu_num, sch)
##########################################################################
...
DEVICE_GRAPH_MULTIPLE = create_device_graph({0: {'name': '/job:localhost/replica:0/task:0/device:XLA_GPU:0', 'memory_size': available_mem, 'type': ''}, 
                                             1: {'name': '/job:localhost/replica:0/task:0/device:XLA_GPU:1', 'memory_size': available_mem, 'type': ''}, 
                                             2: {'name': '/job:localhost/replica:0/task:0/device:XLA_GPU:2', 'memory_size': available_mem, 'type': ''}, 
                                             3: {'name': '/job:localhost/replica:0/task:0/device:XLA_GPU:3', 'memory_size': available_mem, 'type': ''}})


device_list = {0: {'name': '/job:localhost/replica:0/task:0/device:XLA_GPU:0', 'memory_size': available_mem, 'type': ''}, 
               1: {'name': '/job:localhost/replica:0/task:0/device:XLA_GPU:1', 'memory_size': available_mem, 'type': ''}, 
               2: {'name': '/job:localhost/replica:0/task:0/device:XLA_GPU:2', 'memory_size': available_mem, 'type': ''}, 
               3: {'name': '/job:localhost/replica:0/task:0/device:XLA_GPU:3', 'memory_size': available_mem, 'type': ''}}
...
class Profiling:
    ...
    """
    def __init__(self, model, batch_size, gpu=0, rounds=20, input_size=(3, 299, 299)):
        """
    ...
    def recur_function(self, module):
        ...
        for name, sub_module in sub_modules.items():
            ...
            def forward_wrapper(cur_module, *input):
                ...
                #print("Input memory:", input_size)
                #print("Persistent memory:", persistent_memory )
                #print("*"*20)
                ...
                if not(used_mem==512*np.ceil(output_memory/512)):
                    ...
                    print('*'*50)
                '''
                # temp memory used during forward + memory used in backward pass (= gradient per parameter and output)
                # + 0.1 for margin
                temporary_memory = (peak_mem - used_mem) + 2.0*(persistent_memory+output_memory)
    ...
    def run(self):
        ...
        dataset = torchvision.datasets.FakeData(
            size=self.rounds * self.batch_size,
        ...
        for batch_idx, (inp, oup) in enumerate(data_loader):
            ...
            ######################### loss compute ################################################
            try:
                loss = criterion(output, torch.randn(self.batch_size, len(output)).to(self.gpu))
            except: ## For inception
                loss = criterion(output, torch.randn(self.batch_size, len(output[0])).to(self.gpu))
            ##################################################################################
...
def build_graph(model, batch_size, gpu=0, rounds=1, inp_size = (3,299,299)):
    """
    ...
    tester = Profiling(model, batch_size, gpu, rounds, input_size = inp_size)
