...
class SubModuleNode:
    ...
    def __init__(self):
        ...
        # Event to mark end of the output computation
        # Required to synnchronize the streams to send output to child layers
...
class Assign(object):
    ...
    def recur_move_layers_to_gpus(self, module):
        ...
        if len(sub_modules) > 0:
            ...
            # print(module_id)
            # print(gpu_id)
            # for child in this_assigner.model.sub_module_nodes[module_id].children:
            #     print("\t", child, ":", this_assigner.model.sub_module_nodes[child].p)
            ############################################################
            ...
            def modified_forward(self, *inputs):
                ...
                for par in this_assigner.model.sub_module_nodes[module_id].parent:
                    if this_assigner.model.sub_module_nodes[par].p != gpu_id:
                        this_assigner.model.sub_module_nodes[par].event.synchronize()
                ...
                for i, inp in enumerate(input_list):
                    if isinstance(inp, torch.Tensor):
                        ...
                        #with torch.cuda.stream(COMPUTE_STREAM[inp_device]):  ## Stream must be setup on the sending node (see experiment.ipynb, section "Use of CUDA Streams")
                ...
                this_assigner.model.sub_module_nodes[module_id].event.record()
